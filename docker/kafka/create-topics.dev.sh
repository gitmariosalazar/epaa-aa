#!/bin/bash
# =============================================================================
# EPAA-AA — Kafka · Creación de Topics de Desarrollo
#
# Uso:
#   bash create-topics.dev.sh            → Crea todos los topics explícitamente
#   bash create-topics.dev.sh --list     → Lista los topics existentes
#   bash create-topics.dev.sh --delete   → ⚠️  Elimina y recrea todos los topics
#
# Requisitos:
#   - El contenedor "kafka-epaa-dev" debe estar corriendo y HEALTHY
# =============================================================================

set -euo pipefail

CONTAINER="kafka-epaa-dev"
...
echo -e ""
echo -e "${BLUE}${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}${BOLD}║  🐦‍⬛  EPAA-AA · Kafka Dev · Topics          ║${NC}"
echo -e "${BLUE}${BOLD}╚══════════════════════════════════════════════╝${NC}"
echo -e ""
...
if [[ "$HEALTH" != "healthy" ]]; then
  ...
  log_info  "Levanta Kafka primero con: bash deploy.dev.sh"
  exit 1
fi
...
# ─────────────────────────────────────────────────────────────────────────────
if [[ "${1:-}" == "--list" ]]; then
  echo -e "\n${CYAN}${BOLD}Topics existentes en el broker Dev:${NC}"
...
# ─────────────────────────────────────────────────────────────────────────────
# MODO --delete: eliminar todos los topics del proyecto
# ─────────────────────────────────────────────────────────────────────────────
if [[ "${1:-}" == "--delete" ]]; then
  echo -e ""
  echo -e "${RED}${BOLD}╔══════════════════════════════════════════════════╗${NC}"
...
# CREAR TOPICS
# ─────────────────────────────────────────────────────────────────────────────
echo -e "\n${CYAN}${BOLD}Creando topics en Dev...${NC}"
CREATED=0
SKIPPED=0
FAILED=0
...
for ENTRY in "${TOPICS[@]}"; do
  ...
  docker exec "$CONTAINER" kafka-topics \
    ...
    && { log_success "Creado: ${TOPIC_NAME} (particiones=${PARTS})"; ((CREATED++)) || true; } \
...
echo -e ""
echo -e "${CYAN}${BOLD}Topics en el broker Dev:${NC}"
docker exec "$CONTAINER" kafka-topics \
  --bootstrap-server "$BOOTSTRAP" \
  --list
